...
conf_data = np.zeros([n_snapshots, n_atoms, 3])
force_data = np.zeros([n_snapshots, n_atoms, 3])

for i, force_file_str in enumerate(file_list_forces[:n_snapshots]):
    forces = np.loadtxt(force_file_str, delimiter=" ", skiprows=1)  # skips header
...
np.save(dataset_path + '/conf', conf_data, allow_pickle=False)  # save confs
np.save(dataset_path + '/forces', force_data, allow_pickle=False)  # save confs

# load dataset via:
# confs = np.load(dataset_path + '/conf.npy')
# forces = np.load(dataset_path + '/forces.npy')
