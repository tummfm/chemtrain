...
from chemtrain.traj_util import TrajectoryState
...
def init_pot_reweight_propagation_fns(energy_fn_template, simulator_template,
    ...
    replica_exchange = replica_kbt is not None
...
def init_constrained_loss_fn(targets, l_H=1.0, relative_entropy=False):
    """Initializes a constrained maximum entropy loss

    Initializes an augmented Lagrangian [#miele1972]_ to solve the constrained
    optimization problem

    .. math ::
        \\min_\\theta \mathcal L(\\theta)\\ \\text{for}\\ g_k(\\theta) = 0:\\ k = 1,\\ldots,K,

    where the constraints are given by the target observables

    .. math ::
        g_k(\\theta) = \\sqrt{\\frac{\\gamma_k}{\\dim o_k}}\\left(\\hat o_k - \\langle o_k \\rangle_\\theta\\right).

    The augmented lagrangian has the form

    .. math ::
       L(\\theta, \\lambda, \\kappa) = \\mathcal L(\\theta) + \\sum_{k=1}^K \\lambda_k g_k(\\theta) + \\kappa \\sum_{k=1}^K g_k(\\theta) \\cdot g_k(\\theta).

    Args:
        targets: The target dict with ``"gamma"``, ``"target"`` and
            ``"traj_fn"`` for each observable defined in ``quantities``.
        l_H: Scaling factor of the entropy.

    Returns:
        The loss_fn taking trajectories of fluctuating properties,
        computing ensemble averages via the reweighting scheme and
        outputs the loss, penalty, and predicted observables.
    ...
    def loss_fn(quantity_trajs, weights, multipliers):
        predictions = {}

        penalty = 0.0
        if relative_entropy:
            assert "relative_entropy" in targets.keys()
            loss = 0.0
        else:
            loss = -l_H * quantity_trajs.get('entropy', 0.0)

        for target_key in targets:
            quantity = targets[target_key]['traj_fn'](
                quantity_trajs, weights=weights)
            predictions[target_key] = quantity

            # Train on minimizing the relative entropy
            if relative_entropy and target_key == 'relative_entropy':
                loss -= targets[target_key]['gamma'] * quantity
                continue

            # Weight the differences by a pre-determined constant to bring
            # the objectives to equal magnitudes and scale out.
            # Dividing by the square root of the size causes the penalty to
            # be equal to the loss in the unconstrained optimization.
            difference = quantity - targets[target_key]['target']
            difference *= jnp.sqrt(targets[target_key]['gamma'])
            if jnp.ndim(difference) > 0:
                difference /= jnp.sqrt(difference.shape[0])

            loss += jnp.sum(multipliers['lambda'][target_key] * difference)
            penalty += jnp.sum(jnp.square(difference))

        # Regularization via entropy maximization
        debug.print("[Loss] Entropy {entropy} and loss {loss} with unscaled penalty {penalty}", entropy=quantity_trajs.get("entropy", 0.0), loss=loss, penalty=penalty)

        return jnp.asarray([loss, penalty]), predictions
    return loss_fn
